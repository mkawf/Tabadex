...
from ..keyboards import create_currency_keyboard, get_cancel_keyboard
...
) = range(10, 15) # استفاده از یک رنج جدید برای جلوگیری از تداخل با سایر هندلرها
...
async def get_from_currency_and_ask_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # به جای پرسیدن ارز مقصد، مقدار را می‌پرسیم
...
async def get_amount_and_ask_to_currency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # فقط ولیدیشن اولیه که عدد باشد. ولیدیشن نهایی (min/max) بعد از انتخاب ارز مقصد انجام می‌شود
        ...
        return ENTER_AMOUNT # دوباره درخواست کن
...
async def get_to_currency_and_show_preview(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # اکنون می‌توانیم ریت را بگیریم و min/max را چک کنیم
        ...
        if not (min_amount <= float(amount) <= max_amount):
            ...
            # اینجا می‌توانیم کاربر را به مرحله وارد کردن مقدار برگردانیم
            # برای سادگی، فعلا مکالمه را تمام می‌کنیم.
            return ConversationHandler.END
...
async def get_preview_confirmation_and_ask_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data != "preview_confirm":
        ...
        return await cancel_exchange(update, context) # لغو و پاکسازی
...
async def get_address_and_create_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # پاک کردن اطلاعات مکالمه از حافظه موقت
...
async def cancel_exchange(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # پاک کردن تمام کلیدهای احتمالی از حافظه موقت
    keys_to_clear = ['from_currency', 'to_currency', 'amount', 'currencies', 'final_estimated_amount']
    for key in keys_to_clear:
        context.user_data.pop(key, None)
    ...
    if update.callback_query:
        ...
        # ویرایش پیام قبلی بهتر از ارسال پیام جدید است
    ...
    # بازگشت به منوی اصلی
    await show_main_menu(update, context, lang, is_edit=False)
    return ConversationHandler.END
