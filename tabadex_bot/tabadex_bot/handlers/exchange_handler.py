...
# A simple map for Persian currency names to tickers
...
async def start_exchange_conv(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Entry point for the exchange conversation, called from the main menu."""
...
async def get_from_currency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles the selection of the 'from' currency."""
...
async def get_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Gets the amount and asks for the 'to' currency."""
    lang = context.user_data.get("lang", "fa")
    try:
...
async def get_to_currency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Gets the 'to' currency and shows the final preview before asking for address."""
    query = update.callback_query
    await query.answer()
    lang = context.user_data.get("lang", "fa")
...
async def get_preview_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Gets confirmation for the preview and asks for the recipient address."""
    query = update.callback_query
    await query.answer()
    lang = context.user_data.get("lang", "fa")
...
async def get_address_and_create_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Gets the address, creates the transaction, and ends the conversation."""
    lang = context.user_data.get("lang", "fa")
    session = context.db_session
    ...
    try:
        ...
        new_order = await crud.create_order(
            ...
            to_amount_estimated=context.user_data.get("final_estimated_amount", "0"),
            deposit_address=created_tx['depositAddress'],
...
async def prompt_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles the 'Search' button and asks for user input."""
    query = update.callback_query
    await query.answer()
    lang = context.user_data.get("lang", "fa")
...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    results = [c for c in all_currencies if persian_ticker in c['ticker'].lower() or search_term in c['name'].lower()]
