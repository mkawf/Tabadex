...
async def get_name_and_save_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ticker = context.user_data.get('new_address_ticker')
...
async def handle_change_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(get_text("choose_language"), reply_markup=get_language_selection_keyboard(is_reply=True))
...
account_callback_handlers = [
    ...
    CallbackQueryHandler(lambda update, context: show_account_menu(update.callback_query.message, context), pattern="^back_to_account_menu$"),
]