...
# مراحل مکالمه برای ایجاد تیکت جدید
GET_TOPIC, GET_MESSAGE = range(30, 32)
# مرحله مکالمه برای پاسخ به تیکت
GET_REPLY = range(32, 33)
...
async def create_ticket_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    lang = context.user_data.get("lang", "fa")
    # کیبورد ثابت را حذف کرده و کیبورد شیشه‌ای موضوعات را نشان می‌دهیم
    await update.message.reply_text(
...
async def get_ticket_topic(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    topic_key = query.data.split("_", 1)[1] # e.g., "topic_problem_with_order" -> "problem_with_order"
    ...
    await query.edit_message_text(
        ...
        reply_markup=get_cancel_keyboard(lang, "cancel_ticket_creation")
    )
    return GET_MESSAGE

async def get_ticket_message_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """پیام کاربر را دریافت، تیکت را ذخیره و به ادمین‌ها اطلاع می‌دهد."""
    lang = context.user_data.get("lang", "fa")
    ...
    admin_notification_text = get_text("admin_new_ticket_notification", "en").format(
    ...
    # بازگرداندن کیبورد منوی پشتیبانی
...
async def show_ticket_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for msg in ticket.messages:
        sender = "شما" if not msg.is_admin_response else "پشتیبانی"
...
create_ticket_conv = ConversationHandler(
    entry_points=[MessageHandler(filters.Text.regex(f"^{get_text('create_new_ticket_button', 'fa')}$|^{get_text('create_new_ticket_button', 'en')}$"), create_ticket_start)],
    states={
...
support_view_list_handler = MessageHandler(filters.Text.regex(f"^{get_text('view_my_tickets_button', 'fa')}$|^{get_text('view_my_tickets_button', 'en')}$"), view_my_tickets)