...
from ..keyboards import get_support_menu_keyboard, get_support_topics_keyboard, get_user_tickets_keyboard, get_ticket_view_keyboard, get_cancel_keyboard
...
async def show_support_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(get_text("support_menu_title", lang), reply_markup=get_support_menu_keyboard(lang))

async def create_ticket_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    lang = context.user_data.get("lang", "fa")
    await update.message.reply_text(get_text("support_select_topic", lang), reply_markup=get_support_topics_keyboard(lang))
...
async def get_ticket_topic(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(text=get_text("support_enter_message", lang))
    return GET_MESSAGE
...
async def get_ticket_message_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for admin_id in settings.ADMIN_ID_SET:
        ...
        admin_notification_text = get_text("admin_new_ticket_notification", admin_lang).format(user_id=update.effective_user.id, ticket_id=new_ticket.id, title=title)
        try:
...
async def view_my_tickets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tickets:
        ...
        return
    await update.message.reply_text("...", reply_markup=ReplyKeyboardRemove())
    await update.message.reply_text(get_text("my_tickets_title", lang), reply_markup=get_user_tickets_keyboard(tickets, lang))
...
async def reply_to_ticket_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(text=get_text("reply_prompt", lang), reply_markup=get_cancel_keyboard(lang, f"cancel_reply_{ticket_id}"))
    return GET_REPLY
...
async def get_reply_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    class MockQuery:
        def __init__(self, t_id, message): self.data, self.message = f"view_ticket_{t_id}", message
...
async def back_to_ticket_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await view_my_tickets(query.message, context)
...
create_ticket_conv = ConversationHandler(
    ...
    states={GET_TOPIC: [CallbackQueryHandler(get_ticket_topic, pattern="^topic_")], GET_MESSAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_ticket_message_and_save)]},
