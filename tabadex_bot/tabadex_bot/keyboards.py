...
def get_addresses_keyboard(addresses: list[SavedAddress], lang: str) -> InlineKeyboardMarkup:
    """Creates a keyboard for managing saved addresses."""
    keyboard = []
    for addr in addresses:
        # دکمه مشاهده جزئیات به یک دکمه نمایشی تبدیل شده تا خطا ندهد
        row = [
            InlineKeyboardButton(f"{addr.name} ({addr.currency_ticker.upper()})", callback_data="noop"), # noop = no operation
            InlineKeyboardButton("🗑️", callback_data=f"delete_address_{addr.id}"),
        ]
        keyboard.append(row)
